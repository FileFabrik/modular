...
class AutoDiscoveryHelperTest extends TestCase
{
	...
	public function test_it_finds_commands(): void
	{
		...
		$this->helper->commandFileFinder()
					 ->each(function(SplFileInfo $command) use (&$resolved) {
	...
	public function test_it_finds_factory_directories(): void
	{
		...
		$this->helper->factoryDirectoryFinder()
					 ->each(function(SplFileInfo $directory) use (&$resolved) {
	...
	public function test_it_finds_migration_directories(): void
	{
		...
		$this->helper->migrationDirectoryFinder()
					 ->each(function(SplFileInfo $directory) use (&$resolved) {
					 	$resolved[] = str_replace('\\', '/', $directory->getPathname());
					 });
	...
	public function test_it_finds_models(): void
	{
		...
		$this->helper->modelFileFinder()
					 ->each(function(SplFileInfo $file) use (&$resolved) {
	...
	public function test_it_finds_blade_components(): void
	{
		...
		$this->helper->bladeComponentDirectoryFinder()
					 ->each(function(SplFileInfo $file) use (&$resolved_directories) {
					 	$resolved_directories[] = str_replace('\\', '/', $file->getPathname());
					 });

		$this->helper->bladeComponentFileFinder()
					 ->each(function(SplFileInfo $file) use (&$resolved_files) {
	...
	public function test_it_finds_view_directories(): void
	{
		...
		$this->helper->viewDirectoryFinder()
					 ->each(function(SplFileInfo $directory) use (&$resolved) {
					 	$resolved[] = str_replace('\\', '/', $directory->getPathname());
					 });
	...
	public function test_it_finds_lang_directories(): void
	{
		...
		$this->helper->langDirectoryFinder()
					 ->each(function(SplFileInfo $directory) use (&$resolved) {
					 	$resolved[] = str_replace('\\', '/', $directory->getPathname());
					 });
	...
	public function test_it_finds_livewire_component(): void
	{
		...
		$this->helper->livewireComponentFileFinder()
					 ->each(function(SplFileInfo $file) use (&$resolved) {
					 	$resolved[] = str_replace('\\', '/', $file->getPathname());
					 });
		...
		$this->assertContains(
			$this->module1->path('src/'.SupportLivewireSupport::getLivewireLocation().'/TestComponent.php'),
			$resolved
		);

		$this->assertContains(
			$this->module2->path('src/'.SupportLivewireSupport::getLivewireLocation().'/TestComponent.php'),
			$resolved
		);
	}

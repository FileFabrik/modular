...
class MakeModule extends Command
{
    ...
    public function handle()
    {
        ...
        $cfgNamespace = config('app-modules.modules_namespace', 'Modules');
        // todo check laravel prompts
        $module_namespace = text(label      : 'Name of your module?',
                                 placeholder: 'AmazingModule',
                                 default    : $this->argument('name') ?? '',
                                 hint       : 'creates the new module under /app-modules/amazing-module');

        $this->module_name       = Str::kebab($module_namespace);
        $this->class_name_prefix = Str::studly($module_namespace);

        $this->line('module_name is:' . $this->module_name);
        $this->line('class_name_prefix is:' . $this->class_name_prefix);
        ...
        // todo transform later for vendor-namespaced directory
    ...
    {
        return version_compare($this->getLaravel()
                                    ->version(),
                               '8.0.0',
                               '>=')
            ? 'seeders'
    ...
    protected function updateCoreComposerConfig()
    {
        ...
        if (!isset($definition['require'][$this->composer_name])) {
            ...
            $definition['require']                                                     =
                $this->sortComposerPackages($definition['require']);
        }
    ...
    protected function getStubs(): array
    {
        ...
        $composer_stub = version_compare($this->getLaravel()
                                              ->version(),
                                         '8.0.0',
                                         '<')
